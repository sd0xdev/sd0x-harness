...
credential that authorizes a mutating git operation stays exactly what it is today. Projects that
release their own way get a user-owned `rules/git-workflow-project.md` to say so.

## Non-goals

- Adding any Register #4 workflow other than `/deploy-flow` (maintainer decision 2026-09-24), or
  any new kind of credential.
...
- `INV-001`: A menu selection routes to `/smart-commit --execute`, `/push-ci` or `/deploy-flow`; the
  workflow's own plan and AskUserQuestion remain the per-use approval. The selection never is.
...
commit body carrying `Co-Authored-By: Claude …` is rejected with exit 4 on the menu path.
