...
| Override registration (FR-11) | `discretion.md` L3 · `rule-override-pattern` §3.3 (the two-file descriptions) and §3.4 `override_templates` · `install-rules` copy contract · `claude-health` S2.5 #1/#3 (+ conflict check for `## Protected Branches` removal attempts or parse errors — never for omissions — and `## Deploy Workflow` parse errors) · `project-setup` counts · `CLAUDE*.md` `## Rules` · README rule counts | Default |
| `pre-push-gate.sh`, `push-ci`, `epic-merge`, `gh-stack` | Protected test reads the resolver (hook: inlined copy) | Default — but security-bearing; reviewed at `thorough` |
