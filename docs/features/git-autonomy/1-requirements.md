...
- Signal 10 (FR-17/NFR-1): with a user-typed `/goal` active on `feat/x`, a change whose gates pass is committed with no question, through `smart-commit-execute.sh commit` (an AI trailer still exits 4); on `main` the first goal-mode commit recommends a feature branch, then asks to allow `main`; a yes lets later commits of that goal run unasked, a no keeps the menu; an open gate, `## Goal Commit: off`, or a goal the model set unasked → the ordinary menu/approval; no push happens without `/push-ci`'s own approval; after the goal is met or cleared the next commit asks again.
