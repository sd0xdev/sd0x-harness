...
3. Root: the git rules were written as one uniform ceiling. The success criterion is a **two-tier
   ...
   approval and the same attribution guard; protected-branch pushes the harness issues and out-of-workflow operations keep
   today's credentials (a goal-mode commit on a protected branch is FR-17's one exception, after the user allows it; a push made inside an opted-in `run` script is the separate stated-risk case, tech spec § 3.3 step 3) — plus a **project-owned override** so teams with their own deploy flow
...
| While a goal **the user set** is active, the model commits after the gates pass, without a per-use question — on a protected branch only once the user allowed it (FR-17) | Goal-mode push, `/deploy-flow`, or any operation other than commit; a goal-mode commit that skips a gate, the attribution guard, or `/smart-commit`'s runtime validations |
...
| FR-17 | While a goal **the user set** is active — the user typed `/goal <condition>`, or approved a goal the model proposed — the model may run `/smart-commit --execute` **without its per-use AskUserQuestion** once every gate the change classes require reads `pass` at the current digest. Everything else stays: the attribution guard on the same `smart-commit-execute.sh commit` path, sensitive-file exclusion, identity/signing HALTs, the plan printed as a record, never `--ai-co-author`. A `/smart-commit` step that needs the user's judgement (an identity-conflict choice, an ambiguous grouping) still asks. On a feature branch that holds from the first commit. On a protected branch the first goal-mode commit asks: it recommends creating a feature branch, and only if the user declines asks whether commits on that protected branch may proceed for this goal; a yes covers the rest of that goal on that branch, a no falls back to the menu. The credential ends when the goal is met, judged impossible or cleared; a goal the model set without the user's approval never counts. Push, `/deploy-flow` and every other operation keep their own approvals. A project may turn it off (`## Goal Commit` set to `off` (the heading, then a bare `off` line under it)) | Must | Maintainer decision 2026-09-24. A new credential, so a Register #4 change like FR-10 — the grant block, § Efficacy Boundary and their pins move in the same reviewed change (R7) |
...
|----|----------|-------------|--------|
| NFR-1 | Security | Exactly one new credential: after this feature the set of things that authorize a mutating git operation is {workflow AskUserQuestion, `/dev/tty` hook where installed, user message naming the op, **an active user-set goal — for `/smart-commit --execute`, on a protected branch only after the user allowed it for that goal** (FR-17, maintainer 2026-09-24)} | `discretion.md` § Efficacy Boundary and Register #4 change only to add the goal credential with its scope; a test proves a model-set goal, a protected branch the user has not allowed, an open gate and a push each fall back to today's approval |
...
- Signal 10 (FR-17/NFR-1): with a user-typed `/goal` active on `feat/x`, a change whose gates pass is committed with no question, through `smart-commit-execute.sh commit` (an AI trailer still exits 4); on `main` the first goal-mode commit recommends a feature branch, then asks to allow `main`; a yes lets later commits of that goal run unasked, a no keeps the menu; an open gate, `## Goal Commit` set to `off` (the heading, then a bare `off` line under it), or a goal the model set unasked → the ordinary menu/approval; no push happens without `/push-ci`'s own approval; after the goal is met or cleared the next commit asks again.
...
- [x] **Goal-mode commit on protected branches** — decided 2026-09-24: allowed, but the first commit asks — recommend a feature branch first, and only if the user declines ask whether protected-branch commits may proceed for that goal (FR-17).
