...
| Acceptance | Done   | Codex review ✅ Ready after seven rounds on the detector (the regex matchers became one shell-word tokenizer); `--verify-ac` 2026-09-25: 3/3 Complete; AC 3 at Medium (the scaffold choice invoking `/install-rules` is model behaviour set by rule text; that nothing else writes the override is structural), hence Candidate Complete |