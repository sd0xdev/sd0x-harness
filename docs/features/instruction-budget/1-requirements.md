...
|----|-------------|----------|-----------|
| FR-1 | `docs-numbering.md`, `docs-writing.md`, `testing.md` and `testing-project.md` are installed as path-scoped rules (`paths:` frontmatter). Each one's paths must cover everywhere that rule applies today. `docs-writing.md` § Code Comments applies to code files. `testing.md`'s evidence rules apply to request docs and test-review work. `auto-loop.md` reads `testing-project.md` `## Adequacy Mode` at the gate. `testing.md`'s Register #3 "❌ Never" rows stay in that file unchanged; their rule stays in launch context through `discretion.md` Register #3, which states it and is always loaded (§ 9 first question) | Must | Measured: a path-scoped rule is not loaded at launch. A glob narrower than the rule's reach would drop it where it binds |
...
| NFR-2 | Security | Every Anchor rule stays resident in some always-loaded file. Register #3's testing rule stays resident through `discretion.md` (the "❌ Never" rows themselves may path-scope with `testing.md`); the Register #4 git rules and all of `security.md` stay resident as they are | `discretion-tiers.test.js` Register pins unchanged, or changed only with the maintainer's approval |
...
- [x] **testing.md's Anchor rows** — proposed 2026-09-24 and adopted unless the maintainer reverses it: path-scope `testing.md`. The rows stay in `testing.md`, where `discretion-tiers.test.js` pins them, and their rule stays resident through `discretion.md` Register #3 ("security, data-integrity and regression ACs never take manual exceptions"). No Anchor text or pin changes. Reversing it means leaving `testing.md` resident, at a cost of about 5k characters.
