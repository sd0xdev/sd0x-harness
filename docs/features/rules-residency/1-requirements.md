...
| FR-10 | ~~Behaviour after the change is not worse than before on review rounds, deviations and hard incidents, shown before 5.0.0 is released~~ Withdrawn 2026-09-26 (§ 9) | ~~Must~~ Withdrawn | The move must not trade safety or quality for size; the canary is a release gate (maintainer decision 2026-09-25, § 9); release gate withdrawn 2026-09-26 (§ 9) |
...
- FR-10/NFR-1: ~~the tech spec's canary comparison shows no hard incident and soft metrics within margin, and 5.0.0 is tagged only after it reads ship.~~ Superseded 2026-09-26 (§ 9): no canary comparison gates the 5.0.0 tag; the imported log (`canary-log.jsonl`) records no hard incident.
...
- [x] **Canary closeout** — may the canary finish with fewer than 20 baseline changes? Decided 2026-09-26: yes. The 20-change baseline is no longer required; the logged records are imported as they stand, no candidate cohort runs, and the canary no longer gates 5.0.0, which withdraws FR-10's release gate. NFR-1 still states the reliability expectation; none of the six logged changes records a hard incident
