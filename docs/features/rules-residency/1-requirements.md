...
| FR-3 | The plugin-managed resident volume, measured on a rendered fresh install, falls to a stated target | Must | Restores headroom; the target is Open Question 2 |
| FR-4 | User-owned `*-project.md` files are never modified by the migration; they are counted in the total but are not a reduction target | Must | They are the plugin user's customization space |
...
| FR-8 | A change that grows the resident set beyond its limit fails a test that names on-demand placement as the remedy | Should | Prevents the regrowth recorded in the tech spec |
...
| FR-10 | Behaviour after the change is not worse than before on review rounds, deviations and hard incidents | Should | The move must not trade safety or quality for size |
...
| NFR-3 | Maintainability | Plugin-managed resident volume after 5.0 | ≤ the target set in Open Question 2 |
...
| Constraint | Three measures, never interchanged. **Plugin-managed resident**: the rendered template plus plugin rules without `paths:`, excluding every `*-project.md` — the only measure FR-3 targets. **User-owned resident**: the `*-project.md` files — reported, never targeted. **Total resident**: the sum, which is what Claude Code's limit applies to | This analysis; `scripts/instruction-budget.js` "plugin share" also counts shipped override templates, so it is a different quantity |
...
| Assumption | Current state at v4.7.0: total resident 85,673 characters = plugin-managed 78,369 (template 11,328 + plugin rules 67,041) + user-owned 7,304 | Measured 2026-09-25 with `scripts/instruction-budget.js` |
...
- FR-8: adding one character beyond the limit to a resident rule fails the budget test.
- FR-10/NFR-1: the tech spec's canary comparison shows no hard incident and soft metrics within margin.

## 9. Open Questions

- [ ] **Blocking prerequisite** — reconcile tech spec task 7 and the § 3.4 "Rule customization tutorials" row with the `*-project.md` non-goal before 5.0 migration work starts
- [ ] **Anchor-level moves** — may § Push safety, § Efficacy Boundary and § Proactive Offer/Goal mode leave residency behind a compact resident core? Maintainer decision (tech spec § 7 Q1)
- [ ] **Target** — what plugin-managed resident volume should 5.0 reach (the tech spec states ≤ 40,000 bytes for the kernel; this analysis measures characters on a rendered install)?
- [ ] **Canary** — is the tech spec's 20+20-change non-inferiority canary a release gate for 5.0, or a post-release check?
- [ ] **Trigger reliability** — is a resident trigger line enough for ad-hoc push and review work, or does each need a mechanical carrier? Solution concern — suggest `/feasibility-study`
- [ ] **Override contract placement** — can the `*-project.md` precedence rules load only when those files are edited, given the user files themselves stay resident? Solution concern — suggest `/feasibility-study`
- [ ] **Semver** — does any 5.0 change alter an installed project's behaviour enough to need migration notes (the 3.0.0 precedent in `CHANGELOG.md`)?
