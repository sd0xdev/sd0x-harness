...
5.0 では、各セッションの開始時に読み込まれる内容を減らしました。常駐ルールはそれぞれコンパクトな中核だけを残し、詳細な手順は該当する状況になったときに読み込まれる contract に移しました。

| | 4.x | 5.0 |
|---|-----|-----|
| プラグイン管理の常駐ルール（新規 `/project-setup` インストール） | 78,378 文字／721 行 | 49,614 文字／582 行。テストで 50,000 文字・600 行以下に保たれます |
| 詳細な手順：push の承認、レビューループ、スコープ、テスト、ドキュメント、オーバーライド | 常に読み込み | 必要なときに読み込み。`CLAUDE.md` の § Contract Triggers 表が参照先を示します |
| Anchor とゲート | — | 変更なし：禁止事項は同じで、レビュー・`/precommit`・ドキュメントレビューもこれまでと同じタイミングで実行されます |

contract を読み込めない場合、その contract が管轄する操作は停止し、記憶に頼って続行することはありません。

**インストール済みプロジェクトのアップグレード**：プラグインを更新し、`/install-rules --all` を実行してから、プラグインの `CLAUDE.template.md` にある § Contract Triggers を `.claude/CLAUDE.md` にコピーしてください。`*-project.md` の編集は不要です。4.x → 5.0 の完全な対応表は [CHANGELOG.md](CHANGELOG.md#500--rules-load-on-demand) にあります。
